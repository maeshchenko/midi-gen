...
2. **Snapshot-канарейки.** Изменился snapshot в `tests/prng.test.ts` / `tests/code.test.ts` → все выданные пользователям коды сломаны. Либо откатывай правку, либо осознанно bump'ай `CODE_VERSION` в `src/core/code.ts`.
